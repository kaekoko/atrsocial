    ...
    public function get_posts($args = [])
    {
        ...
        switch ($get) {
            case 'newsfeed':
                if (!$this->_logged_in && $query) {
                    ...
                    switch ($system['newsfeed_source']) {
                        ...
                        case 'all_posts':
                            /* get all posts (exclude any post in a group or event and not approved) */
                            $where_query .= " (posts.in_group = '0' AND posts.in_event = '0' AND posts.post_type != 'product') OR (posts.in_group = '1' AND posts.group_approved = '1') OR (posts.in_event = '1' AND posts.event_approved = '1')";
                            break;
                    }
            ...
            case 'posts_profile':
                ...
                if ($this->_logged_in) {
                    ...
                    if ($id == $this->_data['user_id']) {
                        ...
                        $where_query .= "(posts.user_id = $id AND posts.user_type = 'user' AND posts.post_type != 'product')";
                        ...
                        if ($this->friendship_approved($id)) {
                            ...
                            $where_query .= "(posts.user_id = $id AND posts.user_type = 'user' AND posts.in_group = '0' AND posts.in_event = '0' AND posts.privacy != 'me' AND posts.is_hidden = '0' AND posts.is_anonymous = '0' AND posts.post_type != 'product')";
                            /* get target wall posts (from others to the target) [except: hidden posts] */
                            $where_query .= " OR (posts.wall_id = $id AND posts.in_wall = '1' AND posts.is_hidden = '0' AND posts.post_type != 'product')";
                            $where_query .= ")";
                        } else {
                            ...
                            $where_query .= "WHERE (posts.user_id = $id AND posts.user_type = 'user' AND posts.in_wall = '0' AND posts.privacy = 'public' AND posts.is_hidden = '0' AND posts.is_anonymous = '0' AND posts.post_type != 'product')";
                        }
                    }
                } else {
            ...
            case 'products':
                ...
                $me = $this->_data['user_id'];
                $where_query .= "WHERE (posts.post_type = 'product' AND posts.user_id = $me )";
                break;
            ...
            case 'popular':
                $where_query .= "WHERE (posts.privacy = 'public'AND posts.post_type != 'product')"; /* get all popular public posts */
